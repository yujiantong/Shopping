...
<div class="wt1080 middle">
    ...
    <div class="fr">
			...
			<c:forEach items="${requestScope.logList}" var="logistics">
			  <div class="list">
		         <div class="ttt">
		              ...
		              <span>下单时间：
		              	<fmt:formatDate value="${logistics.logisticsOrder.orderDate}" pattern="yyyy-MM-dd"/>
		              </span>
		         </div>
		                <div class="ddd">
		                    <!--------------列表第一部分------------>
		                    ...
		                    <div class="box" style="height: ${logistics.logisticsOrder.orderDetailList.size()*120}px;">
		                        <div style="  margin-top: ${logistics.logisticsOrder.orderDetailList.size()*40}px;">
		                            <c:choose>
				                    	<c:when test="${logistics.logisticsOrder.orderCoupon eq null}">
				                    		<p>${logistics.logisticsOrder.orderAllPrice-logistics.logisticsOrder.orderCoupon.couponReduce}元</p>
		                                    <p style="color: #888888">${logistics.logisticsOrder.orderAllPrice}元（含:运/税费：0.00,优惠卷：0元）</p>
				                    	</c:when>
				                    	<c:otherwise>
				                    		<p>${logistics.logisticsOrder.orderAllPrice-logistics.logisticsOrder.orderCoupon.couponReduce}元</p>
		                                    <p style="color: #888888">${logistics.logisticsOrder.orderAllPrice}元（含:运/税费：0.00,优惠卷：${logistics.logisticsOrder.orderCoupon.couponReduce }元）</p>
				                    	</c:otherwise>
				                    </c:choose>
		                    ...
		                    <div class="box" style="height: ${logistics.logisticsOrder.orderDetailList.size()*120}px;border-right: none;">
		                        <div style="margin-top: ${logistics.logisticsOrder.orderDetailList.size()*40}px;">
		                            <c:if test="${logistics.logisticsStatu.statuId eq 5}">
		                            	<p><a href="javascript:;" class="liji" onclick="payNow()">立即付款</a></p>
      ...
      function cancelOrder(orderId){
           	layer.confirm('确定取消订单？',{btn:['确定','放弃']},
     					function(){
     						tiao1(orderId);
     					},
     					function(){
      ...
      function tiao1(orderId){
	   location.href="${pageContext.request.contextPath}/ordersFont/closeOrder?orderId="+orderId;
	  }
	  ...
	  function cancleO(orderId,reason){
	  		location.href="${pageContext.request.contextPath}/ordersFont/editOrder?reason="+reason+"&orderId="+orderId;
	  }
      function remindOrder(orderId){
      	layer.msg('已提醒发货，请耐心等待~',{time:1020});
      }
      function checkWuLiu(orderId){
      	alert("checkWuLiu"+orderId);
      }
      function makeSureOrder(orderId){
      	layer.confirm('确定收货？',{btn:['确定','放弃']},
     					function(){
     						tiao2(orderId);
     					},
     					function(){
      ...
      function tiao2(orderId){
      	location.href="${pageContext.request.contextPath}/ordersFont/makeSureOrder?orderId="+orderId;
      }
/*       function tiao2(orderId){
      	location.href="${pageContext.request.contextPath}/ordersFont/makeSureOrder?orderId="+orderId;
      } */
